#!/usr/bin/env bash
# Shared cluster-node launcher (Linux). Called by start-master.sh /
# start-slave.sh — keep using those entry points; don't run this directly.
#   Usage: start-node.sh <master|slave> [--build]
set -euo pipefail

ROLE="${1:?Usage: start-node.sh <master|slave> [--build]}"
shift
...
if $BUILD; then
  "$ROOT_DIR/scripts/install-node.sh" "$ROLE"
fi
...
echo "[ok] ${ROLE^} '$NODE_NAME' running. Ctrl+C to stop."
echo "[ok]   ${PIDS_MSG}llm-api PID $LLM_API_PID, hoonbot PID $HOONBOT_PID"
wait
