...
function collectFileUrls(message: any): string[] {
  ...
  if (message.file_url) urls.add(message.file_url);
  if (typeof message.attachments === 'string' && message.attachments.trim()) {
    try {
      const parsed = JSON.parse(message.attachments);
      if (Array.isArray(parsed)) {
        for (const attachment of parsed) {
          if (attachment && typeof attachment.fileUrl === 'string' && attachment.fileUrl.trim()) {
            urls.add(attachment.fileUrl);
          }
        }
      }
    } catch {
      // Keep the legacy file_url fallback for malformed attachment JSON.
    }
  }
...
function cleanupOldFiles() {
  ...
  const oldMessages = queryAll(`
    SELECT id, file_url, attachments FROM messages
    WHERE type IN ('file', 'image')
    AND (file_url IS NOT NULL OR attachments != '[]')
    AND created_at < ?
    AND is_deleted = 0
  `, [cutoffStr]);
  ...
  for (const msg of oldMessages) {
    for (const fileUrl of collectFileUrls(msg)) {
  ...
  for (const msg of oldMessages) {
    run("UPDATE messages SET attachments = '[]' WHERE id = ?", [msg.id]);
  }
