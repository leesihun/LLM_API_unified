...
export async function main() {
  ...
  const uploadsDir = resolveMessengerPath(
  ...
  const publicDir = resolveMessengerPath(
    getMessengerEnv('MESSENGER_PUBLIC_DIR', ''),
  ...
  const clientDistPath = resolveMessengerPath(
    getMessengerEnv('MESSENGER_WEB_DIR', ''),
    path.join(__dirname, '..', '..', 'client', 'dist-web'),
  );
  ...
  const PORT = Number(getMessengerEnv('PORT', '10006')) || 10006;
