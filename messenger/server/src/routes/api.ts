...
function validateMessagePayload(type: unknown, content: unknown, fileUrl: unknown, attachments: MessageAttachment[] = []): string | null {
...
router.post('/send-file', upload.single('file'), (req: Request, res: Response) => {
  ...
  const result = run(
    "INSERT INTO messages (room_id, sender_id, content, type, file_url, file_name, file_size, attachments, mentions) VALUES (?, ?, ?, ?, ?, ?, ?, ?, '[]')",
    [roomId, sender.id, msgContent, msgType, fileUrl, fName, fSize, JSON.stringify(attachments)],
  );
...
router.post('/send-base64', (req: Request, res: Response) => {
  ...
  const result = run(
    "INSERT INTO messages (room_id, sender_id, content, type, file_url, file_name, file_size, attachments, mentions) VALUES (?, ?, ?, 'image', ?, ?, ?, ?, '[]')",
    [roomId, sender.id, msgContent, fileUrl, fName, fSize, JSON.stringify(attachments)],
  );
